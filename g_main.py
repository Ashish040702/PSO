...
class GUI_G:
    def __init__(
        ...
        self.iter_text = Text_G(
            ...
            size=30,
            color=COLOR["white"],
            pos_wrt_center=False,
        )

        self.gbest_text = Text_G(
            text="Global Best: [], Cost: 0",
            pos=(10, 50),
            size=30,
            color=COLOR["white"],
            pos_wrt_center=False,
        ...
        while self.window_open:
            ...
            if current_iteration > 0:
                self.gbest_text.draw_updated_text(
                    self.surface,
                    "Global Best: "
                    + str(self.gbest_evolutions[current_iteration])
                    + ", Cost: "
                    + str(self.gbest_evolutions_annotations[current_iteration]),
                )
                self.graph.draw_graph(
                    self.surface, self.gbest_evolutions[current_iteration]
                )
            else:
