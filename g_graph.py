...
class Edge_G:
    def __init__(self, v1: tuple, v2: tuple, weight: float) -> None:
        ...
        self.weight = weight
        self.weight_text = Text_G(
            text=str(weight),
            pos=self.get_midpoint(),
            size=20,
            color=COLOR["white"],
            pos_wrt_center=True,
        )
    ...
    def draw_edge(self, surface: Type[pygame.Surface], color: Tuple[int, int, int]):
        pygame.draw.line(
            surface=surface, color=color, start_pos=self.start, end_pos=self.end
        )
    ...
    def draw_graph(self, surface: Type[pygame.Surface], solution: List[int]) -> None:
        ...
        if len(solution_copy) > 0:
            ...
            for i in solution_copy:  # handle i + 1
                if i < self.ncount - 1:
                    start, end = (
                        self.nodes[solution_copy[i]],
                        self.nodes[solution_copy[i + 1]],
                    )
